...
class ModelBase(object):
  ...
  def train_one_batch(self, batch_data, task, step):
    self.optimizer.zero_grad()
    loss = self.forward_loss(batch_data, task=task, step=step)
    loss.backward()
    if step in [10000, 20000, 30000, 50000, 70000]:
      lr = self.optimizer.param_groups[0]['lr'] / 2
      self.optimizer.param_groups[0]['lr'] = lr
    #lr = 512 ** (-0.5) * min(step ** (-0.5), step * 10000**(-1.5))
    #self.optimizer.param_groups[0]['lr'] = lr
    self.optimizer.step()
  ...
  def train_one_epoch(self, step, trn_reader, val_reader, TRG, refs, model_dir, log_dir):
    self.train_start()
 
    avg_loss, n_batches = {}, {}
    for task, batch_data in trn_reader:
      loss = self.train_one_batch(batch_data, task, step)
  ...
  def train(self, trn_reader, val_reader, TRG, ref_file, model_dir, log_dir, resume_file=None):
    ...
    for epoch in range(self.config.num_epoch):
      avg_loss, step = self.train_one_epoch(
        step, trn_reader, val_reader, TRG, refs, model_dir, log_dir)
